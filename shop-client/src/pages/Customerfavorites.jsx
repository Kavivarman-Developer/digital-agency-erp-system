...
import { loadShopSettings } from "../utils/settings";
...
const CustomerFavorites = () => {
  ...
  useEffect(() => {
    let active = true;
    loadShopSettings().then((settings) => {
      if (active && !settings.shop_wishlist) navigate("/");
    });
    return () => { active = false; };
  }, [navigate]);
  ...
  return (
    <div style={s.page}>
      ...
      {/* ── Top Nav ─────────────────────────────────────────────────────── */}
      <nav style={s.nav}>
        <Link to="/" style={s.logo}>🛍️ QuickShop</Link>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <Link to="/"           style={s.navLink}>Shop</Link>
          <Link to="/orders"    style={s.navLink}>Orders</Link>
          <Link to="/favorites" style={{ ...s.navLink, color: "#e11d48", fontWeight: 700 }}>
      ...
      {!loading && favorites.length === 0 && !error && (
        <div style={s.emptyState}>
          ...
          <Link to="/" style={s.shopNowBtn}>Browse Shop →</Link>
        </div>
      )}
      ...
      {!loading && favorites.length > 0 && (
        <div style={{ textAlign: "center", marginTop: 40, paddingBottom: 40 }}>
          <Link to="/" style={s.shopNowBtn}>← Continue Shopping</Link>
        </div>
      )}
    </div>
  );
